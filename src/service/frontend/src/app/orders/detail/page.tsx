...
export default function OrderDetailPage() {
  return (
    <Suspense fallback={<div className="text-sm text-zinc-600">로딩 중…</div>}>
      <OrderDetailInner />
    </Suspense>
  );
}
    ...
    <>
      ...
      {!orderId && (
        <div className="mt-6 rounded-xl border bg-white p-6 text-sm text-zinc-600">
          쿼리 파라미터로{" "}
          <code className="rounded bg-zinc-100 px-1">?id=...</code> 를 전달해
          주세요.
        </div>
      )}
        ...
        <div className="mt-6 grid gap-6">
          ...
          <section className="rounded-xl border bg-white p-6">
            ...
            <dl className="mt-4 grid gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <dd>
                  {shipment
                    ? `${shipment.status} (${shipment.shipmentId})`
                    : "-"}
                </dd>
              </div>
              <div className="flex justify-between">
                ...
                <dd>
                  {reservationCount === null ? "-" : `${reservationCount}건`}
                </dd>
              </div>
            </dl>
          ...
          <section className="rounded-xl border bg-white p-6">
            ...
            <p className="mt-2 text-sm text-zinc-600">
              생성 즉시 PENDING으로 저장되고, SQS를 통해 웹훅 이벤트가 비동기
              전달됩니다.
            </p>
