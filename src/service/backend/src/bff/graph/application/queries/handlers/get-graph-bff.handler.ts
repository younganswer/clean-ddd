...
export class GetGraphBffHandler implements IQueryHandler<GetGraphBffQuery> {
  ...
  async execute(query: GetGraphBffQuery): Promise<GraphView | null> {
    ...
    const fetchOrder = async (orderId: string): Promise<OrderView | null> => {
    ...
    const fetchShipmentById = async (
      ...
      if (!id) return null;
      return await this.queryBus.execute<GetShipmentQuery, ShipmentView | null>(
    ...
    while (enqueue.length > 0) {
      ...
      if (item.ref.type === 'ORDER') {
        ...
        const cachedShipmentId = oid ? orderToShipmentId.get(oid) : undefined;
        if (cachedShipmentId) {
          enqueue.push({
