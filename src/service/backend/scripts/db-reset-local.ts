...
function postgresVolumeName(project: string): string {
...
async function dockerComposeDownVolumes(): Promise<void> {
  const project = composeProject();

  // Reset only Postgres container + its named volume.
  // (Keeps other services in the same compose project untouched.)
  await run('docker', [
    'compose',
    '-p',
    project,
...
async function main() {
  ...
  // 4) migrate (includes seed migration)
  const env = {
  ...
  // 4.5) updatedAt trigger (covers nativeUpdate paths too)
  await run(
    'corepack',
  ...
  // 5) seed (replace demo data deterministically)
  await run(
    'corepack',
...
main().catch((error) => {
  ...
  console.error(`db:reset:local 실패: ${message}`);
  process.exitCode = 1;
});
