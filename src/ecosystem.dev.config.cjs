...
const backendCwd = path.join(__dirname, "service", "backend");
const frontendCwd = path.join(__dirname, "service", "frontend");
...
module.exports = {
	apps: [
		// HTTP API (cluster)
		{
			...
			env_local: {
				...
				SQS_OUTBOX_QUEUE_URL:
					"http://localhost:45666/000000000000/OutboxDispatchQueue.fifo",
		...
		// Frontend dev server (fork)
		{
			...
			script: "corepack",
			args: ["pnpm", "exec", "next", "dev", "-p", "8080"],
			...
			// Keep logs under backend/logs to avoid creating a new root logs directory.
