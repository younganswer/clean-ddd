...
function OrderDetailInner() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setOrder(data);
      setShipment(ship);
      setReservationCount(Array.isArray(reservations) ? reservations.length : null);
    } catch (e: any) {
      ...
      {order && (
        <div className="mt-6 grid gap-6">
          ...
          <section className="rounded-xl border bg-white p-6">
            ...
            <dl className="mt-4 grid gap-2 text-sm">
              <div className="flex justify-between">
                <dt className="text-zinc-600">배송(Shipment)</dt>
                <dd>{shipment ? `${shipment.status} (${shipment.shipmentId})` : "-"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-zinc-600">재고 예약</dt>
                <dd>{reservationCount === null ? "-" : `${reservationCount}건`}</dd>
              </div>
            </dl>
            <p className="mt-2 text-xs text-zinc-500">결제 성공 이벤트가 처리되면 자동으로 채워집니다.</p>
          </section>

          <section className="rounded-xl border bg-white p-6">
            <h2 className="text-lg font-semibold">결제 인텐트 생성(시뮬레이터)</h2>
            <p className="mt-2 text-sm text-zinc-600">
              생성 즉시 PENDING으로 저장되고, SQS를 통해 웹훅 이벤트가 비동기 전달됩니다.
            </p>
