...
export class Migration20260210000000 extends Migration {
  override async up(): Promise<void> {
    this.addSql(`alter table if exists "orders" add column if not exists "items" jsonb not null default '[{"sku":"SKU-001","quantity":1}]'::jsonb;`);
    ...
    this.addSql(`create index if not exists "shipments_created_at_idx" on "shipments" ("created_at");`);
    ...
    this.addSql(`create index if not exists "inventory_items_updated_at_idx" on "inventory_items" ("updated_at");`);
    ...
    this.addSql(`create index if not exists "inventory_reservations_created_at_idx" on "inventory_reservations" ("created_at");`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql('alter table if exists "orders" drop column if exists "items";');
  }
}
