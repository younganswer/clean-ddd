...
export class OutboxProducer {
  constructor(
    @Inject(IOutboxRepositorySymbol) private readonly outboxRepo: IOutboxRepository,
  ...
  async emit(
    ...
    const inferredMessageGroupId =
      options?.messageGroupId ?? (typeof payload.orderId === 'string' && payload.orderId ? payload.orderId : 'outbox');

    if (disableDelaySeconds && typeof delaySeconds === 'number' && delaySeconds > 0) {
      setTimeout(() => {
