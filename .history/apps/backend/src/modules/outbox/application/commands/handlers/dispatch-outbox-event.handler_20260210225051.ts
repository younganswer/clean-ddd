...
export class DispatchOutboxEventHandler
  implements ICommandHandler<DispatchOutboxEventCommand>
{
  constructor(
  ...
  async execute(command: DispatchOutboxEventCommand): Promise<void> {
    ...
    const messageGroupId = String(command.messageGroupId ?? '').trim() || 'outbox';
