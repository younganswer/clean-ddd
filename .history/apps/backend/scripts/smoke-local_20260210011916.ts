...
async function main() {
  ...
  try {
    ...
    const shipment = await waitForTruthy('Shipment', async () =>
      fetchJson('GET', `http://localhost:3000/api/v1/shipments/by-order/${encodeURIComponent(orderId)}`),
    );
    const reservations = await waitForTruthy<any[]>('InventoryReservation', async () =>
      fetchJson(
        'GET',
        `http://localhost:3000/api/v1/inventory/reservations?orderId=${encodeURIComponent(orderId)}`,
      ),
    );
