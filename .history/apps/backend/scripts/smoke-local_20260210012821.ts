...
async function waitForTruthy<T>(label: string, fn: () => Promise<T | null | undefined>): Promise<T> {
...
async function main() {
  ...
  await sqs.send(
    new SendMessageCommand({
      QueueUrl: queueUrl,
      MessageBody: JSON.stringify({ smoke: true, at: new Date().toISOString() }),
  ...
  const forceFrontend = (process.env.SMOKE_FORCE_FRONTEND ?? 'false') === 'true';
  const frontendLockPath = path.join(repoRoot(), 'apps', 'frontend', '.next', 'dev', 'lock');
  ...
  try {
    ...
    const detail = await fetchJson('GET', `${apiBaseUrl}/orders/${encodeURIComponent(orderId)}`);
    ...
    const shipment = await waitForTruthy('Shipment', async () =>
      fetchJson('GET', `${apiBaseUrl}/shipments/by-order/${encodeURIComponent(orderId)}`),
    );
    const reservations = await waitForTruthy<any[]>('InventoryReservation', async () =>
      fetchJson(
        'GET',
    ...
    console.log({
      ...
      intent,
      shipment,
      reservationsCount: Array.isArray(reservations) ? reservations.length : null,
